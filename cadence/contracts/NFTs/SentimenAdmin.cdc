...
pub contract SentimenAdmin{
    ...
    pub resource Admin {
        ...
        // at the beginning of the SentimenPack contract.
        //
        ...
        pub fun addPackType(packType: UInt64, numberOfCards: UInt64) {
            pre {
                SentimenPack.packTypes[packType] == nil:
                    "This pack type already exists!"
            }
            // Adds this pack type
            SentimenPack.packTypes[packType] = SentimenPack.PackType(_packType: packType, _numberOfCards: numberOfCards)
        }
        ...
        pub fun mintPacks(packType: UInt64, numberOfPacks: UInt64, packNumbers: [UInt64]) {
            ...
            let adminPackCollection = SentimenAdmin.account.borrow<&SentimenPack.Collection{SentimenPack.IPackCollectionPublic}>(from: /storage/sentimenPackCollection)!

            var i: UInt64 = 0
            while i < numberOfPacks {
                let newPack <- SentimenPack.createPack(packNumber: packNumbers[i], packType: packType)
    ...
    init() {
        self.account.save(<- create Admin(), to: /storage/sentimenAdmin)
    }
}